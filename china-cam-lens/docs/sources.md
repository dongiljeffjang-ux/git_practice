...
> 목적: China CAM/AAM Lens의 소스 스택 확정. 판정·접근조건·제약을 검증한 결과. finlight.me / webapi.cninfo.com.cn 두 도메인은 검증 환경 프록시 차단으로 직접 fetch 불가 → 검색 기반 확인, 해당 비용/티어는 "미확인"으로 표기(계약 전 직접 견적 필요).
